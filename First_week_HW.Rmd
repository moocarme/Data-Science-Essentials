...
Separating question via income level. There is a factor coloumn in the dataset
```{r, warning = F}
trade.byage <- trade %>% filter(country == 'United States') %>% select(age, Q1) %>% mutate(age.bracket = factor(ifelse(age<25, '18-24', ifelse(age < 40, '25-39', ifelse(age < 60, '41-59', ifelse(age < 80, '60-79', 'Over 80')))))) %>% gather(Question, Answer,-age,-age.bracket) %>% group_by(age.bracket, Question, Answer ) %>% summarise(Total.counts = n()) %>% mutate(rel.freq = Total.counts/sum(Total.counts))
...
```{r, warning = F}
trade.byage <- trade %>% filter(country == 'United States') %>% select(age, Q1) %>% mutate(age.bracket = factor(ifelse(age<25, '18-24', ifelse(age < 40, '25-39', ifelse(age < 60, '41-59', ifelse(age < 80, '60-79', 'Over 80')))))) %>% gather(Question, Answer,-age,-age.bracket) %>% group_by( Question, Answer, age.bracket ) %>% summarise(Total.counts = n()) %>% mutate(rel.freq = Total.counts/sum(Total.counts))
